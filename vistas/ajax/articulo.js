...
function init() {
    ...
    //cargamos los items al el select
    $.post("../controladores/articulo.php?op=selectCategoria", function (r) {
        $("#idcategoria").html(r);
...
function limpiar() {
    ...
    $("#idcategoria").val("")
    $("#codigo").val("");
    $("#nombre").val(""); //el objeto cuyo id tiene nombre le enviaremos un valor vacio
    $("#descripcion").val("");  //el objeto cuyo id tiene descripcion le enviaremos un valor vacio
    $("#stock").val("");
    $("#imagenactual").val("");
    $("#imagenmuestra").hide();
...
function listar() {
    $tabla = $('#tbllistado').dataTable({
        ...
        "ajax": {
            url: '../controladores/articulo.php?op=listar', //mediante ajax vamos a obtener valores de la url que hemos descrito
...
function guardaryeditar(e) {
    ...
    $.ajax({
        ...
        url: "../controladores/articulo.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(idarticulo) {
    ...
    $.post("../controladores/articulo.php?op=mostrar", { idarticulo: idarticulo }, function (data) {
        ...
        $("#box-title").text("Editar Articulo");
...
function desactivar(idarticulo) {
    var tabla = $('#tbllistado').DataTable();
    Swal.fire({
        title: '¿Estás seguro de desactivar la artículo?',
        text: "",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.post("../controladores/articulo.php?op=desactivar", { idarticulo: idarticulo }, function (e) {
...
function activar(idarticulo) 
{
    var tabla = $('#tbllistado').DataTable();
    Swal.fire({
        title: '¿Estás seguro de activar la artículo?',
        text: "",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        ...
        if (result.isConfirmed) {
            $.post("../controladores/articulo.php?op=activar", {idarticulo:idarticulo }, function (e) {
...
//funcion para generar el codigo de barra
function generarbarcode()
{
    //capturo el id=codigo y lo almaceno en una variable 
    codigo =$("#codigo").val();

    //con la funcion JsBarcode deseo que me muestre el codigo
    //en el id=barcode, y a su vez quiero que me convierta codigo en una barra de codigo
